...
/// </summary>
public class MarkDownChunker(ITextTokenizer? tokenizer = null)
{
    ...
    /// <returns>List of chunks.</returns>
    public List<string> Split(string text, MarkDownChunkerOptions options)
    {
    ...
    internal static SeparatorTypes NextSeparatorType(SeparatorTypes separatorType) => separatorType switch
    {
        ...
        SeparatorTypes.NotASeparator or _ => throw new ArgumentOutOfRangeException($"{nameof(SeparatorTypes.NotASeparator).ToLower()} doesn't have a next separator type."),
    };
