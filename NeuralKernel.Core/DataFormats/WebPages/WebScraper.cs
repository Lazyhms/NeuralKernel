...
public sealed class WebScraper : IWebScraper, IDisposable
{
    ...
    public WebScraper(ILoggerFactory loggerFactory, HttpClient? httpClient = null)
    {
        _httpClient = httpClient ?? new HttpClient();
        _httpClient.DefaultRequestHeaders.UserAgent.ParseAdd("");

        _log = loggerFactory.CreateLogger<WebScraper>();
    }
    ...
    private async Task<WebScraperResult> GetAsync(Uri url, CancellationToken cancellationToken = default)
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            {
                _log.LogError("Error while fetching page {AbsoluteUri}, status code: {StatusCode}", url.AbsoluteUri, response.StatusCode);
            }
        ...
        {
            _log.LogDebug("URL '{AbsoluteUri}' fetched, content type: {contentType}", url.AbsoluteUri, contentType);
        }
        ...
        else
        {
            ...
            await content.CopyToAsync(s2, cancellationToken);
            bytes = s2.ToArray();
        }
